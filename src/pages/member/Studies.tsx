...
import { useStudies } from "@/data/queries";
  ...
  return (
    <SiteLayout>
      <header className="text-left">
        <h1 className="font-display text-3xl">Estudos</h1>
      ...
      <section className="mt-8 grid gap-6 md:grid-cols-2">
          ...
          <Card key={d.id} className="overflow-hidden">
            <CardHeader>
              <CardTitle className="font-display">
                <NavLink to={`/membro/estudos/${d.id}`} className="brand-underline">
                  {d.title}
                </NavLink>
              </CardTitle>
            </CardHeader>
        ...
        {(!filtered || filtered.length === 0) && (
          <div className="rounded-3xl border bg-card p-6 text-left text-muted-foreground">Nenhum estudo encontrado.</div>
        )}
      </section>
    </SiteLayout>
  );
